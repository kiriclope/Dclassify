...
class bolasso(BaseEstimator, ClassifierMixin):
    def __init__(
        ...
        self.fs_idx_ = None  # or appropriate default value
    ...
    def get_bag_coef(self):
        ...
        for i_boot in range(self.n_boots):
            ...
            coef = model_boot.named_steps["model"].coef_[0]

            # selector = SelectFromModel(model_boot.named_steps["model"], prefit=True)
            ...
            if "filter" in self.clf.named_steps.keys():
                ...
                self.bag_coef_[i_boot] = model_boot.named_steps["model"].coef_[0]
    ...
    def fit(self, X, y):
        self.clf.named_steps["model"].penalty = self.penalty

        if self.penalty=='elasticnet':
            self.clf.named_steps["model"].l1_ratios = [0.95]

        self.clf.named_steps["model"].n_jobs = -1
    ...
    def get_coef(self):
        ...
        if "filter" in self.clf.named_steps.keys():
            ...
            coefs[mask] = self.clf.named_steps["model"].coef_[0]
            ...
            coefs = self.coef_[self.fs_idx_]
            coefs[idx_keep] = self.clf.named_steps["model"].coef_[0]
            self.coef_[self.fs_idx_] = coefs

        elif "pca" in self.clf.named_steps.keys():
            n_idx = self.clf.named_steps["pca"].n_components_
            coefs = self.coef_[self.fs_idx_]
            coefs[:n_idx] = self.clf.named_steps["model"].coef_[0]
            self.coef_[self.fs_idx_] = coefs

        else:
            self.coef_[self.fs_idx_] = self.clf.named_steps["model"].coef_[0]

        self.coef_[np.abs(self.coef_) <= 1e-5] = 0

        self.intercept_ = self.clf.named_steps["model"].intercept_
